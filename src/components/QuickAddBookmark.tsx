    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
      ...
      <form onSubmit={handleQuickAdd} className="flex flex-col sm:flex-row gap-2">
        ...
        <button
          ...
          className="btn btn-primary flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed py-3 sm:py-2"
        >
          <Plus className="h-4 w-4 mr-2" />
          <span className="hidden sm:inline">{loading ? 'Adding...' : 'Add'}</span>
          <span className="sm:hidden">{loading ? 'Adding...' : 'Quick Add'}</span>
        </button>
      </form>
